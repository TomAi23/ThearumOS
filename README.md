<div align="center">
  <img src="public/logo.png" width="160" height="160" alt="ThearumOS Logo" style="border-radius: 32px; box-shadow: 0 20px 40px rgba(0,0,0,0.4);">
  
  # ThearumOS v1.0.0
  **The Supreme Life Operating System — Local, Private, Insanely Fast.**
  
  [![Privacy First](https://img.shields.io/badge/Privacy-Local%20Only-emerald?style=for-the-badge&logo=lock)](https://github.com/thearumrin/ThearumOS)
  [![Security](https://img.shields.io/badge/Security-AES--256--GCM-blue?style=for-the-badge)](https://github.com/thearumrin/ThearumOS)
  [![Platform](https://img.shields.io/badge/Platform-macOS%20|%20Windows-lightgrey?style=for-the-badge&logo=apple)](https://github.com/thearumrin/ThearumOS)
</div>

---

## 💎 The Vision
ThearumOS is a high-performance intelligence engine designed for the elite professional. It is built on the philosophy that your life data is sacred. While other apps sell your data to the cloud, ThearumOS keeps everything on your silicon.

## 🚀 Key Evolutionary Features
- **Total Privacy**: Zero network requests. Zero tracking. Zero cloud.
- **AES-256-GCM Encryption**: Military-grade vault for your Finances and Contacts.
- **Apple-Inspired Aesthetic**: Borderless dynamic logos, a sleek Notes sidebar handle, and vibrant 'Apple Intelligence' tri-color typography.
- **Centralized Settings Hub**: Manage profiles, adjust Glassmorphism intensity, switch System Accents, and handle raw data backups.
- **Intelligence Engine v1.0.0**: A hyper-efficient NLP parser that turns natural language into structured life data across all 12 modules.
- **30-Step Undo Engine**: Total resilience. Every action is reversible with `⌘Z`.

---

## 🛠 12 Intelligence Modules
| Module | Focus | Intelligence |
|:---:|:---|:---|
| **Dashboard** | Command Center | Live stats & Daily Score |
| **Tasks** | Execution | Priority-aware todo engine |
| **Projects** | Construction | Kanban-style roadmap |
| **Finance** | Wealth | Encrypted Income/Expense/Investment tracker |
| **Health** | Vitality | Biometric logs (Weight/Sleep/Water/Gym) |
| **Habits** | Consistency | Visual streak & routine management |
| **Learning** | Growth | Personal Knowledge Base |
| **People** | Network | Encrypted CRM & Birthday engine |
| **Notes** | Second Brain | AI-assisted thought indexing |
| **Journal** | Reflection | Mood analytics & Memory threading |
| **Goals** | Vision | Long-term objective tracking |
| **Calendar** | Timeline | Visual commitment mapping |

---

## 💻 Developer Command Center

### Prerequisites
- **Node.js**: v18 or later.
- **Hard Drive**: Local storage required for your personal vault.

### Installation
```sh
# Clone the intelligence
git clone https://github.com/thearumrin/ThearumOS.git

# Initialize systems
npm install

# Deploy locally
npm run dev
```

### Production Construction
```sh
# Build for macOS (App Store ready)
npm run electron:build -- --mac

# Build for Windows (Portable)
npm run electron:build -- --win
```

---

## 🔒 Security Architecture
1. **Context Isolation**: Renderer processes are fully sandboxed via secure IPC bridges.
2. **Device-Specific Keys**: Encryption keys are local, unique, and never synchronized.
3. **OS-Level Persistence**: Data resides in `~/ThearumOS_Data`, separate from application logic.

---
<p align="center">
  Crafted by <b>Thearum Rin</b>. Insanely great productivity awaits.
</p>
